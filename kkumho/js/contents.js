$(document).ready(function(){
    ...
    //console.log($('.ctn_tstory').length)
    if($('.ctn_tstory').length > 0){
   
        let scrolling //브라우저가 스크롤된 값
        let window_h //브라우저 높이
        let obj_name = $('.ctn_tstory .his_photo')
        let obj_photo 
        let obj_photo_top
        let obj_photo_show 
        let obj_nav = $('.ctn_tstory .his_nav')
        let obj_nav_area = $('ctn_tstory')
        let obj_nav_start // 보이기 시작하는 스크롤값
        let obj_nav_end // 보이지 않는 마지막 스크롤값
        ...
        let menuName = $('.ctn_tstory .his_nav')  // 상단에 고정할 메뉴 영역 선택자
        let menuItem = $('.ctn_tstory .his_nav ul li') // data-link 값을 준 클릭할 요소의 선택자
        let sectionName
        let moveTop
        let areaTop
        let areaH
